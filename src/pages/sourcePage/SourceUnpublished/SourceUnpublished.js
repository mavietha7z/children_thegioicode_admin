...
function SourceUnpublished() {
    ...
    useEffect(() => {
        document.title = 'Quản trị website - Danh sách mã nguồn chưa duyệt';

        setLoading(true);

        const fetch = async () => {
            const result = await requestAuthGetSources(page, 'unpublished', id);
    ...
    const handleToggleSource = async (id) => {
        ...
        const result = await requestAuthUpdateSource(id, 'published', {});
        ...
        if (result.status === 401 || result.status === 403) {
            ...
            notification.success({
                message: 'Thông báo',
                description: 'Xuất bản mã nguồn thành công',
            });
        } else {
            notification.error({
                message: 'Thông báo',
...
export default SourceUnpublished;
